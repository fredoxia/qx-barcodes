...
pageNav.fn = function(page,totalPage){                
    ...
    document.chainListForm.action="chainTransferJSPAction!listToChainStore";
    document.chainListForm.submit();
};

function selectToChainStore(chainId, chainName){
	 window.opener.selectToChainStore(chainId, chainName); 
	 window.close();
}
</script>
</head>
<body>
    <%@ include file="../../common/Style.jsp"%>
   <s:form action="/actionChain/chainTransferJSPAction!listToChainStore" method="POST" name="chainListForm" id="chainListForm" theme="simple"> 
	<%@ include file="../../common/pageForm.jsp"%>
	<s:hidden id="fromChainId" name="formBean.transferOrder.fromChainStore.chain_id"/>
		    ...
		    <tr>
		      <td colspan="3">
					...
					<table width="100%"  align="left" class="OuterTable" id="org_table">
				      ...
				      <s:iterator value="uiBean.toChainStores" status="st" id="chainStore" >
						    <tr class="InnerTableContent" <s:if test="#st.even">style='background-color: rgb(255, 250, 208);'</s:if>>	      
						      ...
						      <td><a href='#' onclick="selectToChainStore(<s:property value="#chainStore.chain_id"/>,'<s:property value="#chainStore.chain_name"/>');"><img src='<%=request.getContextPath()%>/conf_files/web-image/editor.gif' border='0'/></a></td>
						    </tr>
				       </s:iterator>	
				       <s:if test="uiBean.toChainStores.size == 0">
