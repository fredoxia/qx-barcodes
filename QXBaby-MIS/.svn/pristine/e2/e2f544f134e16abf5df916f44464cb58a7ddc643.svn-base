...
function downloadInventory(quarterId){
	$("#quarterId").attr("value", quarterId);
...
<body>
	...
	<s:hidden name="formBean.reportType" id="reportType"  value="2"/> 
			      ...
			      <td colspan="7">
						...
						<table width="100%"  align="left" class="OuterTable" id="org_table">
						  ...
						  <tbody id="orderTablebody">
						      <s:iterator value="uiBean.levelTwoInventoryItem" status = "st" id="ci" >
							      ...
							      <td align="center"><s:property value="#ci.quarter.quarter_Name"/></td>					      
							      ...
							      <td align="center"><a href='#' onclick="getLevelThree(<s:property value="#ci.year.year_ID"/>,<s:property value="#ci.quarter.quarter_ID"/>)"><img src='<%=request.getContextPath()%>/conf_files/web-image/editor.gif' border='0'  title="查看"/></a>
							      				     <a href='#' onclick="downloadInventory(<s:property value="#ci.quarter.quarter_ID"/>)"><img src='<%=request.getContextPath()%>/conf_files/easyUI/themes/icons/filesave.png' border='0' title="下载"/></a></td>
							     </tr>
							   </s:iterator>  
						  </tbody>
						  <s:if test="uiBean.levelTwoInventoryItem.size >0">
							  <tr class="PBAInnerTableTitale">
							    ...
							    <td align="right"><s:if test="#session.LOGIN_CHAIN_USER.containFunction('purchaseAction!seeCost')"><s:property value="uiBean.inventoryItem.totalCostAmt"/></s:if>
							                      <s:else>-</s:else>
							    </td>
