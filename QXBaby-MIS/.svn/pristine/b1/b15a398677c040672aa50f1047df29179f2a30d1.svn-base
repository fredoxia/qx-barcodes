...
pageNav.fn = function(page,totalPage){                
    ...
    document.chainListForm.action="chainMgmtJSP!listChainStore";
    document.chainListForm.submit();
};

function selectChainStore(chainId, chainName){
	 window.opener.selectChainStore(chainId, chainName); 
	 window.close();
}
</script>
</head>
<body>
    <%@ include file="../../common/Style.jsp"%>
   <s:form action="/actionChain/chainMgmtJSP!listChainStore" method="POST" name="chainListForm" id="chainListForm" theme="simple"> 
